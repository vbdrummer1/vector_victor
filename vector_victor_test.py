...
def test_vector_shape_vectors():
    """vector_shape takes a vector or matrix and return a tuple with the
    number of rows (for a vector) or the number of rows and columns
    (for a matrix.)"""
    assert vector_shape([1]) == (1,)
    assert vector_shape(m) == (2,)
    assert vector_shape(v) == (3,)
...
@raises(ShapeError)
def test_vector_add_checks_vector_shapes():
    """vector_shape rule: the vectors must be the same size."""
    vector_add(m, v)
...
@raises(ShapeError)
def test_vector_sub_checks_vector_shapes():
    """vector_shape rule: the vectors must be the same size."""
    vector_sub(m, v)
...
@raises(ShapeError)
def test_vector_sum_checks_vector_shapes():
    """vector_shape rule: the vectors must be the same size."""
    vector_sum(v, w, m, y)
#
#
...
@raises(ShapeError)
def test_dot_checks_vector_shapes():
    """vector_shape rule: the vectors must be the same size."""
    dot(v, m)
